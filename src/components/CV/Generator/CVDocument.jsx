...
export const MyDocument = ({ data, sectionsOrder, bulletPoints, skillsOrder }) => {
    return (
        <Document>
            <Page size='A4' style={{ fontFamily: 'Times-Roman', padding: 30 }} wrap>
                ...
                {sectionsOrder.map(item => {
                    if (item.id === 'workExperience') {
                        ...
                        return <SkillsToolsInterests key={item.id} skillsToolsInterests={data.skillsToolsInterests} skillsOrder={skillsOrder} bulletPoints={bulletPoints.main} wrap={false} />
                    } else {
